...
def main():
    ...
    parser.add_argument("--path", default="tests/data", type=str, help="Path to the dataset directory.")
    ...
    if found_videos:
        ...
        for video_path in found_videos:
            try:
                ...
                if result_list:
                    all_results.extend(result_list)
                    logger.info(f"[OK] {video_path.name} -> {len(result_list)} faces detected.")
                else:
