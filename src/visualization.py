    ...
    def plot_charts(self):
        ...
        try:
            ...
            if 'race' in self.df.columns:
                ...
                p_race.add_tools(HoverTool(tooltips=[("Race", "@race"), ("Count", "@count")]))
            else:
            ...
            if 'gender' in self.df.columns:
                ...
                if len(gender_counts) == 2:
                    ...
                    gender_counts['color'] = Category20c[len(gender_counts)] if len(gender_counts) > 2 else ['gray']
            ...
            if 'age' in self.df.columns:
                ...
                labels = ["0-7", "8-14", "15-21", "22-28", "29-35", "36-42", "43-49", "50-56", "57-63", "64-70", "70+"]

                self.df['age_group'] = pd.cut(self.df['age'], bins=bins, labels=labels, right=False)
                
                age_data = self.df['age_group'].value_counts().sort_index().reset_index()
                age_data.columns = ['age_group', 'count']
                
                source_age = ColumnDataSource(age_data)

                p_age = figure(x_range=labels, height=350, title="Age Distribution",
                               toolbar_location=None, tools="")
                
                p_age.vbar(x='age_group', top='count', width=0.9, source=source_age,
                           line_color='white', fill_color="#40916c")
                
                p_age.add_tools(HoverTool(tooltips=[("Age Group", "@age_group"), ("Count", "@count")]))
            else:
